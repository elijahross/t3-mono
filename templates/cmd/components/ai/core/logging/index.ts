...
export class LLMLogger {
  ...
  private constructor() {
    ...
    // Setup log file path (only create directory if file logging is enabled)
    ...
    if (this.shouldLogToFile()) {
      try {
        ...
        console.warn(`Failed to create log directory ${logDir}: ${err.message}`);
      }
    }
        ...
        new winston.transports.Console({
          format: winston.format.combine(
            ...
            winston.format.printf((info) => {
              ...
              return `${timestamp} [LLM] ${provider}/${model} | In: ${inputTokens} Out: ${outputTokens} | ${latencyMs}ms | $${(cost as number)?.toFixed(4) || "0.0000"}`;
            })
          ),
        })
      );
  ...
  async log(entry: LLMLogEntry): Promise<void> {
    ...
    if (this.shouldLogToTerminal() || this.shouldLogToFile()) {
      this.logger.info({ message: "LLM call", ...entry });
    }
